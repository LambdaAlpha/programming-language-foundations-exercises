...
Proof.
intros. intros st st'. split; intro H.
- inversion H; subst.
  + apply E_IfFalse.
  ...
  + apply E_IfTrue; try assumption.
    simpl. rewrite H5. reflexivity.
- inversion H; subst; apply f_equal with (f:=negb) in H5; simpl in H5; rewrite <- negb_involutive_reverse in H5; simpl in H5.
  + apply E_IfFalse; try assumption.
  + apply E_IfTrue; try assumption.
Qed.
(** [] *)
...
intros. split. 
- intros. inversion H0.
  + unfold bequiv in H. rewrite H in H5. inversion H5.
  + exfalso. apply (WHILE_true_nonterm b c st st' H H0).
- intros. exfalso. cut (bequiv true BTrue). intros. apply (WHILE_true_nonterm true SKIP st st' H1 H0). unfold bequiv. simpl. intros. reflexivity.
Qed.
...
Proof.
intros. intros st st'. split; intros.
- inversion H; subst. inversion H2; subst. apply E_Seq with st'1. assumption. apply E_Seq with st'0. assumption. assumption.
- inversion H; subst. inversion H5; subst. apply E_Seq with st'1. apply E_Seq with st'0. assumption. assumption. assumption.
Qed.
(** [] *)
...
- assert (Hx : st' =[ x ::= e ]=> (x !-> (aeval st' e); st')). { apply E_Ass. reflexivity. } unfold aequiv in H. rewrite <- (H st') in Hx. rewrite t_update_same in Hx. assumption.
- unfold aequiv in H. rewrite <- (H st). rewrite t_update_same. apply E_Skip.
Qed.
(** [] *)
...
Proof.
intros. intros st st'. split; intros H1; inversion H1; subst;
apply E_Seq with st'0; (apply H; apply H4; fail) || (apply H0; apply H7).
Qed.
(** [] *)
...
Theorem CIf_congruence : forall b b' c1 c1' c2 c2',
  ...
  cequiv (TEST b THEN c1 ELSE c2 FI)
         (TEST b' THEN c1' ELSE c2' FI).
Proof.
intros. intros st st'. split; intro Hif; (inversion Hif; subst; [
(apply E_IfTrue; [
...
Proof.
intros. induction a; simpl; inversion H; intuition.
- inversion H; subst. simpl. unfold t_update. apply eqb_string_false_iff in H1. rewrite H1. reflexivity.
Qed.
...
(apply E_Seq with st'0; [
  ...
  (* only case var is not solved by auto *)
...
Proof.
unfold not. unfold cequiv. intros. destruct (H empty_st empty_st). assert (empty_st =[ WHILE true DO SKIP END ]=> empty_st). { apply H1. apply E_Skip. } apply (loop_never_stops empty_st empty_st). unfold loop. apply H2.
Qed.
(** [] *)
...
Proof.
left. unfold cequiv. unfold pXY. unfold pYX. intros. split; intros; inversion H; subst; inversion H2; subst; inversion H5; subst; rewrite t_update_permute.
...
Proof.
right. unfold not. unfold cequiv. unfold ptwice. unfold pcopy. intros. 
destruct (H empty_st (Y !-> 2; X !-> 1)). clear H. clear H1.
assert (empty_st =[ (HAVOC X;; Y ::= X)%imp ]=> (Y !-> 2; X !-> 1)). { 
  apply H0. apply E_Seq with (X !-> 1).
    - apply (E_Havoc empty_st X 1).
    - apply (E_Havoc _ _ 2). }
clear H0. 
inversion H; subst; clear H. 
inversion H2; subst; clear H2. 
inversion H5; subst; clear H5. 
simpl in H3. rewrite t_update_eq in H3. 
assert (forall s, (Y !-> n; X !-> n) s = (Y !-> 2; X !-> 1) s). {
  rewrite H3. reflexivity. }
assert ((Y !-> n; X !-> n) X = (Y !-> 2; X !-> 1) X) by (apply H).
assert ((Y !-> n; X !-> n) Y = (Y !-> 2; X !-> 1) Y) by (apply H).
assert (Y <> X). { rewrite <- eqb_string_false_iff. auto. }
rewrite (t_update_neq _ _ Y X _ H2) in H0. rewrite (t_update_neq _ _ Y X _ H2) in H0. rewrite t_update_eq in H0. rewrite t_update_eq in H0. subst. 
rewrite t_update_eq in H1. rewrite t_update_eq in H1. inversion H1.
Qed.
(** [] *)
