...
- eapply hoare_consequence. eassumption. assumption. assumption.
Qed.
(** [] *)
...
Lemma wp_is_precondition: forall c Q,
  {{wp c Q}} c {{Q}}.
Proof.
unfold hoare_triple. intros. apply H0. assumption.
Qed.
(** [] *)
...
Proof.
unfold wp. intros. apply (H st s' H1). assumption.
Qed.
...
Proof.
  ...
  - apply H_If.
    + eapply IHc1. intros st st' H1 H2. destruct H2. inversion H0. apply (HT st st').
      * apply E_IfTrue.
        -- assumption.
        -- assumption.
      * assumption.
    + eapply IHc2. intros st st' H1 H2. destruct H2. apply bassn_eval_false in H0. apply (HT st st').
      * apply E_IfFalse.
        -- assumption.
        -- assumption.
      * assumption.
  - (* 网上随便找的, 重点是要用到 (wp (WHILE b DO c END) Q) *) unfold hoare_triple in HT.
