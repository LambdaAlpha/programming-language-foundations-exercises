...
Proof.
  ...
  - unfold normal_form. intros H. destruct H. inversion H; subst. inversion H1.
  - unfold value. intros H. destruct H.
    + inversion H.
...
Proof.
  unfold normal_form. induction t; intros; inversion H; subst; inversion H0; subst; try (intros H1; destruct H1; inversion H1). apply IHt. 
  - right. assumption.
  - inversion H1; subst. exists t1'. assumption.
Qed.

Lemma value_is_nf1 : forall t,
  value t -> step_normal_form t.
Proof.
  unfold normal_form. intros. inversion H. 
  - inversion H0; subst; intros H1; destruct H1; inversion H1.
  - induction H0.
    + intros H0. destruct H0. inversion H0.
    + intros H1. destruct H1. apply IHnvalue.
      * right. assumption.
      * inversion H1; subst. exists t1'. assumption.
Qed.
...
Theorem step_deterministic:
  deterministic step.
Proof with eauto.
  unfold deterministic. intros x y1. generalize dependent x.  induction y1; intros; inversion H; subst; inversion H0; subst; try reflexivity; try solve_by_invert.
  - exfalso. apply value_is_nf with (scc t1)...
  - rewrite (IHy1_1 t1 t1' H3 H6). reflexivity.
  - exfalso. apply value_is_nf with (scc zro)...
  - rewrite (IHy1 t1 t1' H3 H2). reflexivity.
  - exfalso. apply value_is_nf with (scc (scc y1))...
  - exfalso. apply value_is_nf with (scc y2)...
  - rewrite (IHy1 t1 t1' H3 H2). reflexivity.
  - exfalso. apply value_is_nf with (scc t0)...
  - rewrite (IHy1 t1 t1' H3 H2). reflexivity.
Qed.
...
Proof with auto.
  ...
  induction HT;
    ...
    - inversion HE; subst. apply IHHT in H0. apply T_Scc. assumption.
...
Proof with eauto.
  intros t t' T HT HE.
  generalize dependent T.
  induction HE; intros T HT; inversion HT; subst...
  inversion H1; subst...
Qed.
(** [] *)
...
(** **** Exercise: 2 stars, standard (variation2)  
   ...
   this rule?  For each one that does, give a counter-example.
      - Determinism of [step]
            becomes false. test tru t2 t3 -> t2 / t3
      - Progress
            remains true
      - Preservation
            remains true
...
(** **** Exercise: 2 stars, standard, optional (variation3)  
   ...
   this rule?  For each one that does, give a counter-example.
      - Determinism of [step]
            becomes false. test (iszro zro) (iszro zro) t3 -> test tru (iszro zro) t3 / test (iszro zro) tru t3
      - Progress
            remains true
      - Preservation
            remains true
...
(** **** Exercise: 2 stars, standard, optional (variation5)  
   ...
   this rule?  For each one that does, give a counter-example.
      - Determinism of [step]
            remains true
      - Progress
            becomes false. test zro tru fls
      - Preservation
            remains true
...
(** **** Exercise: 2 stars, standard, optional (variation6)  
   ...
   this rule?  For each one that does, give a counter-example.
      - Determinism of [step]
            remains true
      - Progress
            becomes false. test (prd zro) tru fls
      - Preservation
            becomes false. ~ |- zro \in Bool
