...
Theorem step_deterministic :
  deterministic step.
Proof.
unfold deterministic. intros x y1. generalize dependent x. induction y1; intros; inversion H0; subst; inversion H; subst; try solve_by_inverts 2.
- reflexivity.
- rewrite (IHy1_1 t1 t1' H3 H1). reflexivity.
- rewrite (IHy1_2 t2 t2' H8 H2). reflexivity.
Qed.
(** [] *)
...
Proof.
exists (P (P (C 0) (C 0)) (C 0)). split.
- apply v_funny.
- unfold normal_form. unfold not. intros. apply H. exists (P (C 0) (C 0)). apply ST_Plus1. apply ST_PlusConstConst.
Qed.
End Temp1.
...
exists (C 0). split.
- apply v_const.
- unfold normal_form. unfold not. intros. apply H. exists (P (C 0) (C 0)). apply ST_Funny.
Qed.
...
- unfold normal_form. intros H. destruct H. inversion H; subst. inversion H3.
Qed.
...
Theorem step_deterministic :
  deterministic step.
Proof.
unfold deterministic. intros x y1. generalize dependent x. induction y1; intros; inversion H; subst; inversion H0; subst; try solve_by_invert; try reflexivity.
rewrite (IHy1_1 t1 t1' H3 H6). reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  generalize dependent y2. induction P11.
  - intros. inversion P21; subst.
    + reflexivity.
    + apply nf_is_value in P12. inversion P12; subst. inversion H.
  - intros. apply IHP11.
    + assumption.
    + inversion P21; subst.
      * exfalso. apply P22. exists y. assumption.
      * assert (y = y0). { apply (step_deterministic _ _ _ H H0). } rewrite H2. assumption.
    + assumption.
Qed.
(** [] *)
...
- intros. inversion H; subst; clear H. apply IHt1 in H2; clear IHt1. apply IHt2 in H4; clear IHt2. apply multi_trans with (P (C n1) t2).
...
Proof.
  ...
  - intros. inversion H; subst. inversion H0; subst. apply E_Plus. assumption. apply IHHs. assumption.
Qed.
(** [] *)
...
Proof.
  intros. unfold normal_form_of in H. destruct H. induction H.
  - apply nf_is_value in H0. inversion H0. exists n. split. reflexivity. apply E_Const.
  - specialize (IHmulti H0). destruct IHmulti. destruct H2. exists x0. split. assumption. apply step__eval with y. assumption. assumption.
Qed.
(** [] *)
...
Proof.
  induction t; intros.
  - split; intros.
    + simpl in H. rewrite H. apply E_Const.
    + simpl. inversion H; subst. reflexivity.
  - split; intros.
    + simpl in H. rewrite <- H. apply E_Plus.
      * apply IHt1. reflexivity.
      * apply IHt2. reflexivity.
    + simpl. inversion H; subst. apply IHt1 in H2. apply IHt2 in H4. rewrite H2. rewrite H4. reflexivity.
Qed.
(** [] *)
...
Theorem combine_deterministic:
  deterministic step.
Proof.
  unfold deterministic. intros x y1. generalize dependent x. induction y1; intros; inversion H; subst; inversion H0; subst; try solve_by_inverts 2; try reflexivity.
  - rewrite (IHy1_1 t1 t1' H3 H5). reflexivity.
  - rewrite (IHy1_2 t2 t2' H5 H7). reflexivity.
  - rewrite (IHy1_1 t1 t1' H3 H6). reflexivity.
Qed.

Check strong_progress.
Theorem combine_non_strong_progress:
  exists t, ~ value t /\ forall t', ~ (t --> t').
Proof.
exists (test (C 0) tru fls). split.
- intros H. inversion H.
- intros. intros H. inversion H; subst. inversion H4.
Qed.
...
Proof.
  ...
  eapply multi_step. apply CS_Par2. apply CS_SeqStep.
    apply CS_Ass.
  eapply multi_step. apply CS_Par2. apply CS_SeqFinish.
  rewrite H. rewrite plus_comm. simpl. 
  eapply multi_refl.
Qed.
(** [] *)
...
Proof.
  ...
  - intros. specialize (IHn st H). destruct IHn. destruct H0. eapply ex_intro. split.
    + set (par_body_n__Sn _ _ H1). eapply multi_trans.
      * apply H0.
      * apply m.
    + split.
      * rewrite t_update_eq. reflexivity.
      * rewrite t_update_neq. destruct H1.
        -- assumption.
...
Theorem normalize_ex : exists e',
  ...
  -->* e' /\ value e'.
Proof.
  eapply ex_intro. split. normalize. auto.
Qed.
(** [] *)
...
Theorem normalize_ex' : exists e',
  ...
  -->* e' /\ value e'.
Proof.
  exists (C 6). split. normalize. auto.
Qed.
(** [] *)
