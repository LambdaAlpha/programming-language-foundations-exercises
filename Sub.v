...
(** **** Exercise: 3 stars, standard, optional (count_supertypes)  
    ...
    (A | Top) * (C->C | C->Top | Top) 2 * 3 * 2 + 2 + 3 + 1 + 1
    [] *)
...
Proof with auto.
  ...
  remember Bool as V.
  induction Hs...
  - specialize (IHHs2 HeqV). rewrite <- IHHs2. apply IHHs1. rewrite IHHs2. assumption.
  - inversion HeqV.
  - inversion HeqV.
Qed.
(** [] *)
...
Proof with eauto.
  ...
  induction Hs; intros...
  - apply IHHs2 in HeqV. destruct HeqV. destruct H. destruct H. apply IHHs1 in H. destruct H. destruct H. destruct H. exists x1. exists x2. split.
    + assumption.
    + destruct H1. destruct H0. split.
      * apply S_Trans with x...
      * apply S_Trans with x0...
  - inversion HeqV.
...
Lemma canonical_forms_of_arrow_types : forall Gamma s T1 T2,
  ...
  exists x S1 s2,
     s = abs x S1 s2.
Proof with eauto.
  intros g s T1 T2 Hty Hv. remember (Arrow T1 T2) as T. generalize dependent T2. generalize dependent T1. 
  induction Hty; intros; try solve_by_invert.
  - exists x. exists T11. exists t12. reflexivity.
  - rewrite HeqT in H. pose proof (sub_inversion_arrow _ _ _ H). destruct H0. destruct H0. destruct H0. apply IHHty with (T1:= x)( T2:= x0)...
