...
Inductive substi (s : tm) (x : string) : tm -> tm -> Prop :=
  ...
  | s_abs2 : forall y t1 s1 T,
      x <> y ->
      substi s x t1 s1 ->
      substi s x (abs y T t1) (abs y T s1)
  | s_app : forall t1 s1 t2 s2,
      substi s x t1 s1 ->
      substi s x t2 s2 ->
      substi s x (app t1 t2) (app s1 s2)
  | s_tru :
      substi s x tru tru
  | s_fls :
      substi s x fls fls
  | s_if : forall t1 s1 t2 s2 t3 s3,
      substi s x t1 s1 ->
      substi s x t2 s2 ->
      substi s x t3 s3 ->
      substi s x (test t1 t2 t3) (test s1 s2 s3)
.
...
Proof.
  intros s x0 t. induction t; intros; split; intros; inversion H; subst; auto.
  - destruct (eqb_spec x0 s0).
    + simpl. rewrite e. rewrite <- eqb_string_refl. constructor.
    + simpl. assert (eqb_string x0 s0 = false). apply eqb_string_false_iff. assumption. rewrite H. apply s_var2. assumption.
  - simpl. rewrite <- eqb_string_refl. reflexivity.
  - simpl. assert (eqb_string x0 s0 = false). apply eqb_string_false_iff. assumption. rewrite H0. reflexivity.
  - simpl. eapply s_app.
    + apply IHt1. reflexivity.
    + apply IHt2. reflexivity.
  - apply IHt1 in H2. apply IHt2 in H4. simpl.  rewrite H2. rewrite H4. reflexivity.
  - destruct (eqb_spec x0 s0).
    + rewrite e. simpl. rewrite <- eqb_string_refl. apply s_abs1.
    + simpl. assert (eqb_string x0 s0 = false). apply eqb_string_false_iff. assumption. rewrite H. apply s_abs2.
      * assumption.
      * apply IHt. reflexivity.
  - simpl. rewrite <- eqb_string_refl. reflexivity.
  - simpl. assert (eqb_string x0 s0 = false). apply eqb_string_false_iff. assumption. rewrite H0. specialize (IHt s1). rewrite <- IHt in H5. rewrite H5. reflexivity.
  - simpl. apply s_if.
    + apply IHt1. reflexivity.
    + apply IHt2. reflexivity.
    + apply IHt3. reflexivity.
  - simpl. rewrite <- (IHt1 _) in H3. rewrite <- (IHt2 _) in H5. rewrite <- (IHt3 _) in H6. rewrite H3. rewrite H5. rewrite H6. reflexivity.
Qed.
(** [] *)
...
Example typing_example_2_full :
  empty |-
    ...
    (Arrow Bool (Arrow (Arrow Bool Bool) Bool)).
Proof.
  apply T_Abs. apply T_Abs. apply (T_App Bool). 
  - apply T_Var. apply update_eq.
  - apply T_App with Bool.
    + apply T_Var. apply t_update_eq.
    + apply T_Var. apply t_update_neq. apply eqb_neq. simpl. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  induction T11.
  - inversion H1.
  - inversion H1; subst. apply IHT11_1. assumption.
Qed.
(** [] *)
