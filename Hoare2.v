...
- eapply hoare_consequence_post.
  + eapply hoare_while. apply hoare_consequence_pre with (assn_sub X (X - 2) (fun st => parity (st X) = parity m)).
    * apply hoare_asgn.
    * intros st H. destruct H. unfold assn_sub. rewrite t_update_eq. simpl. unfold bassn, beval, aeval in H0. apply leb_complete in H0. rewrite parity_ge_2.
      -- assumption.
      -- assumption.
  + intros st H. destruct H. unfold bassn, beval, aeval in H0. apply not_true_is_false in H0. apply leb_nle in H0. rewrite <- H. symmetry. apply parity_lt_2. assumption.
...
(* FILL IN HERE 
    ...
    WHILE ~(X = m) DO
      {{ Z = 2^X /\ Y = 2^(X + 1) - 1 /\ ~(X = m) }}
      {{ 2 * Z = 2^(X + 1) /\ Y + 2 * Z = 2^(X + 1 + 1) - 1 }}
      Z ::= 2 * Z;;
      {{ Z = 2^(X + 1) /\ Y + Z = 2^(X + 1 + 1) - 1 }}
      Y ::= Y + Z;;
      {{ Z = 2^(X + 1) /\ Y = 2^(X + 1 + 1) - 1 }}
      X ::= X + 1
      {{ Z = 2^X /\ Y = 2^(X + 1) - 1 }}
...
- eapply hoare_consequence_pre.
  + apply hoare_asgn.
  + intros st H. unfold assn_sub. rewrite t_update_eq. simpl. intuition.
- intros. intros st H'. unfold hoare_triple in H.
  assert (aeval st (Y + 1) = st Y + 1). { simpl. reflexivity. }
  specialize (H st (X !-> aeval st (Y + 1); st) (E_Ass st (Y + 1) (st Y + 1) X H0) H').
  rewrite t_update_eq in H. simpl in H. intuition.
Qed.
(** [] *)
...
- apply hoare_asgn.
- intros. intros st H'. apply H with st.
...
Lemma hoare_havoc_weakest : forall (P Q : Assertion) (X : string),
  ...
  P ->> havoc_pre X Q.
Proof.
intros. unfold havoc_pre. intros st HP n. apply H with st.
- apply E_Havoc.
- assumption.
Qed.
End Himp2.
...
- destruct (st X).
  + omega.
  + simpl in H. simpl. rewrite mult_assoc_reverse. rewrite sub_0_r. rewrite mul_succ_l. rewrite plus_comm. assumption.
...
- intros. destruct n.
  + simpl. reflexivity.
  + clear H. simpl (Init.Nat.pred (S n)) in IHn. simpl (Init.Nat.pred (S (S n))). rewrite (plus_comm (S n) 1) in IHn. simpl (1 + S n) in IHn. rewrite <- IHn.
    * rewrite (plus_comm (S (S n)) 1). simpl. reflexivity.
    * omega.
Qed.
(** [] *)
...
Definition dfib (n : nat) : decorated :=
    {{ fun st => True }} ->>
    {{ fun st => 1 = fib (1 - 1) /\ 1 = fib 1 /\ 1 <= 1 }}
    X ::= 1
    {{ fun st => 1 = fib (st X - 1) /\ 1 = fib (st X) /\ 1 <= st X }};;
    Y ::= 1
    {{ fun st => st Y = fib (st X - 1) /\ 1 = fib (st X) /\ 1 <= st X }};;
    Z ::= 1
    {{ fun st => st Y = fib (st X - 1) /\ st Z = fib (st X) /\ 1 <= st X }};;
    WHILE ~(X = n + 1) DO
      {{ fun st => st Y = fib (st X - 1) /\ st Z = fib (st X) /\ 1 <= st X /\ ~(st X = n + 1) }} ->>
      {{ fun st => st Z = fib (st X) /\ st Z + st Y = fib (st X + 1) /\ 1 <= st X + 1 }}
      T ::= Z
      {{ fun st => st T = fib (st X) /\ st Z + st Y = fib (st X + 1) /\ 1 <= st X + 1 }};;
      Z ::= Z + Y
      {{ fun st => st T = fib (st X) /\ st Z = fib (st X + 1) /\ 1 <= st X + 1 }};;
      Y ::= T
      {{ fun st => st Y = fib (st X) /\ st Z = fib (st X + 1) /\ 1 <= st X + 1 }};;
      X ::= X + 1
      {{ fun st => st Y = fib (st X - 1) /\ st Z = fib (st X) /\ 1 <= st X }}
    END
    {{ fun st => st Y = fib (st X - 1) /\ st Z = fib (st X) /\ 1 <= st X /\ st X = n + 1 }} ->>
...
Proof.
intros. verify.
- rewrite <- pred_of_minus. apply fib_eqn. omega.
- assert (st X + 1 - 1 = st X) by omega. rewrite H. reflexivity.
- assert (n + 1 - 1 = n) by omega. rewrite H. reflexivity.
Qed.
(** [] *)
