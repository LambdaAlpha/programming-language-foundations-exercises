...
Proof.
  ...
  - right. inversion Ht; subst. specialize (IHt1 _ H2). specialize (IHt2 _ H4). destruct IHt1.
    + set (canonical_forms_fun _ _ _ H2 H). destruct e. destruct H0. destruct IHt2. 
      * exists ([x0 := t2] x1). rewrite H0. apply (ST_AppAbs x0 T11 x1 t2 H1).
      * destruct H1. exists (app t1 x2); auto.
    + destruct H. exists (app x0 t2). auto.
  - right. inversion Ht; subst. specialize (IHt1 _ H3). specialize (IHt2 _ H5). specialize (IHt3 _ H6). destruct IHt1.
    + apply (canonical_forms_bool _ H3) in H. destruct H.
      * exists t2. rewrite H. auto.
      * exists t3. rewrite H. auto.
    + destruct H. exists (test x0 t2 t3). auto.
Qed.
(** [] *)
...
Corollary typable_empty__closed : forall t T,
    ...
    closed t.
Proof.
  unfold closed. intros. intros H1. apply (free_in_context x0 t T empty H1) in H. destruct H. inversion H.
Qed.
(** [] *)
...
Proof.
  ...
  - pose proof (progress _ _ Hhas_type) as H1. apply IHHmulti.
    + apply preservation with x0; auto.
    + assumption.
    + assumption.
Qed.
(** [] *)
...
Theorem unique_types : forall Gamma e T T',
  ...
  T = T'.
Proof.
  intros g e. generalize dependent g. induction e; intros; inversion H; subst; inversion H0; subst; auto.
  - rewrite H3 in H4. inversion H4. auto.
  - specialize (IHe1 g _ _ H4 H5). specialize (IHe2 _ _ _ H6 H8). subst. inversion IHe1; subst. trivial.
  - specialize (IHe _ _ _ H6 H7). rewrite IHe. trivial.
  - specialize (IHe2 _ _ _ H7 H10). rewrite IHe2. trivial.
Qed.
(** [] *)
...
and the following typing rule:
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        becomes false. tru --> tru / zap
      - Progress
        becomes false. forall t, ~ zap --> t /\ ~ value t
      - Preservation
        becomes false. tru --> zap, empty |- zap \in Arrow /\ ~ empty |- t \in Arrow
*)
...
(** **** Exercise: 2 stars, standard (stlc_variation2)  
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        becomes false. app (\x:A. x) tru --> tru / app foo tru
      - Progress
        remains true
      - Preservation
(       becomes false. (\x:A. x) --> foo --> tru
*)
...
(** **** Exercise: 2 stars, standard (stlc_variation3)  
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        remains true
      - Progress
        becomes false. app (app idBBBB idBB) idB
      - Preservation
        remains true
*)
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation4)  
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        becomes false. (test tru then idB else idB) --> tru / idB
      - Progress
        remains true
      - Preservation
        becomes false. (test tru then idB else idB) --> tru
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation5)  
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        remains true.
      - Progress
        becomes false. ~ value (t1 t2) /\ forall t, ~ (t1 t2) --> t
      - Preservation
        remains true
...
(** **** Exercise: 2 stars, standard, optional (stlc_variation7)  
    ...
    false, give a counterexample.
      ...
      - Determinism of [step]
        remains true
      - Progress
        becomes false. app (\x:Bool.x) (\x:Bool.t)
      - Preservation
        becomes false. idBB \x:Bool.x --> \x:Bool.x
