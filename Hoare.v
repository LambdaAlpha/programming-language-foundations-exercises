...
Theorem hoare_asgn_fwd :
  ...
  {{fun st => P (X !-> m ; st)
           /\ st X = aeval (X !-> m ; st) a }}.
Proof.
intros. unfold hoare_triple. intros. destruct H0. inversion H; subst. split.
- rewrite t_update_shadow. rewrite t_update_same. assumption.
- rewrite t_update_eq. rewrite t_update_shadow. rewrite t_update_same. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros a P.
  unfold hoare_triple. intros. inversion H; subst. exists (st X). split.
  - rewrite t_update_shadow. rewrite t_update_same. assumption.
  - rewrite t_update_eq. rewrite t_update_shadow. rewrite t_update_same. reflexivity.
Qed.
(** [] *)
...
- eapply hoare_consequence_pre.
  + eapply hoare_asgn.
  + unfold assn_sub, t_update, assert_implies. simpl. intros st [_ H].
    symmetry. apply le_plus_minus_r. inversion H. apply leb_complete. assumption.
- eapply hoare_consequence_pre.
  + eapply hoare_asgn.
  + unfold assn_sub, t_update, assert_implies. simpl. intros.
    reflexivity.
Qed.
(** [] *)
...
Theorem hoare_if1 : forall P b c,
  {{fun st => P st /\ bassn b st}} c {{P}} ->
  {{P}} (IF1 b THEN c FI)%imp {{P}}.
Proof.
...
Proof.
intros. intros st st' H1 H2. generalize dependent P.
remember (REPEAT c UNTIL b END)%imp as wcom eqn:Heqwcom.
induction H1; intros;
try (inversion Heqwcom); subst; clear Heqwcom.
- split. apply (H2 st st'). assumption. assumption. apply bexp_eval_true. assumption.
- apply IHceval2 with (fun st => Q st /\ ~(bassn b st)). reflexivity. assumption. split.
  apply (H1 st st'). assumption. assumption.
...
Proof.
unfold hoare_triple. intros. intros. inversion H; subst. apply H0.
Qed.
...
Theorem assert_assume_differ : exists P b Q,
  ...
  /\ ~ ({{P}} ASSERT b {{Q}}).
Proof.
exists (fun st => True). exists BFalse. exists (fun st => True). split.
- unfold hoare_triple. intros. inversion H; subst. inversion H2.
- unfold hoare_triple. unfold not. intros. destruct (H empty_st RError).
...
Theorem assert_implies_assume : forall P b Q,
  ...
  -> ({{P}} ASSUME b {{Q}}).
Proof.
unfold hoare_triple. intros. inversion H0; subst. assert (st =[ ASSERT b ]=> RNormal st). { apply E_AssertTrue. assumption. } apply (H st (RNormal st) H2 H1).
Qed.
...
Theorem hoare_assert: forall P b,
  {{fun st => P st /\ bassn b st}} ASSERT b {{fun st => P st /\ bassn b st}}.
Proof.
unfold hoare_triple. intros. inversion H; subst.
- exists st. destruct H0. split. reflexivity. split. assumption. apply bexp_eval_true. assumption.
- destruct H0. inversion H1; subst. rewrite H2 in H4. inversion H4.
Qed.
...
Proof.
unfold hoare_triple. intros. inversion H; subst.
exists st. split. reflexivity. split. assumption. apply bexp_eval_true. assumption.
Qed.
...
Example assert_assume_example:
  ...
  {{fun st => True}}.
Proof.
apply hoare_seq with (fun st => True /\ bassn (X = 1) st).
- apply hoare_seq with (fun st => True /\ bassn (X = 2) st).
  + apply hoare_consequence_post with (fun st => True /\ bassn (X = 2) st).
    * apply hoare_assert.
    * intros st H. apply I.
  + apply hoare_consequence_pre with (assn_sub X (X + 1) (fun st => st X = 2)).
    * apply hoare_consequence_post with (fun st => st X = 2).
      -- apply hoare_asgn.
      -- intros st H. split. apply I. unfold bassn. unfold beval. unfold aeval. rewrite H. auto.
    * intros st [_ H]. unfold assn_sub. rewrite t_update_eq. inversion H; subst. apply eqb_eq in H1. simpl. rewrite H1. reflexivity.
- apply hoare_assume.
Qed.
